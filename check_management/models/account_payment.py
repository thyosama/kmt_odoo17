...
class AccountPayment(models.Model):
    ...
    cash_journal_id = fields.Many2one('account.journal', string="Cash Journal",
                                      domain=[('is_treasury_feed', '=', True)], copy=False)
    account_transfer_id = fields.Many2one('account.account', string="Account Transfer")
    ...
    feed_confirm_journal_id = fields.Many2one('account.move', string="feed confirm Journal",readonly=1)
    feed_journal_id = fields.Many2one('account.move', string="feed Journal",readonly=1)
    ...
    type_cheq = fields.Selection([
        ('account_feed', 'Account'),
        ('cash_feed', 'Treasury'),
        ('account_transfer', 'Account Transfer'),
        ('send_che', 'Send Cheque'),
        ('recieve_chq', 'Recieve Cheque')], string="Type")
    ...
    #     for item in self:
    #         if item.journal_cheque:
    #             if item.type_cheq=='cash_feed':
    #                 if item.journal_cheque.currency_id:
    #                     item.currency_id = item.journal_cheque.currency_id.id
    #                 else:
    #                     item.currency_id = item.journal_cheque.company_id.currency_id.id

    def check_due_date_notify(self):
        # payments_sent = self.env['account.payment'].search([('state','=','posted'),('type_cheq','=','send_che')])
        payments = self.env['account.payment'].search([('state', 'in', ('posted', 'sent'))])
        for payment in payments:

            if payment.type_cheq == 'send_che':
                if payment.effective_date:
                    diff = relativedelta(payment[0]['effective_date'], date.today())
                    days = diff.days
                    print(days)
                    if days <= 7 and days > 0:
                        ...
                        activity_ins.create(
                            {'res_id': payment.id,
                             'res_model_id': self.env['ir.model'].search([('model', '=', 'account.payment')]).id,
                             'res_model': 'account.payment',
                             'activity_type_id': 3,
                             'summary': 'There IS Sent Cheque %s  Reminder one Day for Due Date Please Check Due Date at %s' % (
                             payment.name, payment.effective_date),
                             'note': _(
                                 'There IS Sent Cheque %s  Reminder one Day for Due Date Please Check Due Date at %s' % (
                                 payment.name, payment.effective_date)),
                             'date_deadline': fields.Date.today(),
                             'activity_category': 'default',
                             # 'form_view_ref': 'check_management.view_account_payment_cheque_send',
                    ...
                    if days <= 7 and days > 0:
                        ...
                        activity_ins.create(
                            {'res_id': payment.id,
                             ...
                             'res_model': 'account.payment',
                             'activity_type_id': 3,
                             'summary': 'There IS Received Cheque %s  Reminder one Day for Due Date Please Check Due Date at %s' % (
                                 payment.name, payment.effective_date),
                             'note': _(
                                 'There IS Received Cheque %s  Reminder one Day for Due Date Please Check Due Date at %s' % (
                                     payment.name, payment.effective_date)),
                             'date_deadline': fields.Date.today(),
                             'activity_category': 'default',
                             'previous_activity_type_id': False,
                             # 'form_view_ref': 'check_management.view_account_payment_cheque',
    ...
    @api.onchange('journal_collection')
    def change_journal_get_currency(self):
        for rec in self:
            if self.type_cheq == 'send_che':
                self.journal_id = self.journal_collection.id
            if rec.journal_collection:
                rec.currency_id = rec.journal_collection.currency_id.id if rec.journal_collection.currency_id else rec.env.company.currency_id.id

    @api.onchange('cheque_bank')
    def get_acount_bank(self):
        banks = []
        if self.cheque_bank:
            bank_ids = self.env['res.partner.bank'].search(
                [('bank_id', '=', self.cheque_bank.id)])
            for bank_id in bank_ids:
                banks.append(bank_id.id)
            domain = {'bank_account': [('id', 'in', banks)]}

            return {'domain': domain}

    @api.onchange('bank_account')
    def get_acount_documnet(self):
        documnets = []
        if self.bank_account:
            bank_ids = self.env['cheque.document'].search(
                [('bank_account', '=', self.bank_account.id)])
            for bank_id in bank_ids:
                documnets.append(bank_id.id)
            domain = {'document_id': [('id', 'in', documnets)]}
    ...
    def onchange_currency(self):
        ...
        if self.journal_cheque:
            if self.journal_cheque.currency_id:
                self.currency_id = self.journal_cheque.currency_id.id
            else:
                self.currency_id = self.journal_cheque.company_id.currency_id.id
    ...
    def create(self, vals):
        ...
        if not vals.get("compnay_id", False):
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        if self.type_cheq!='cash_feed' :

            if self.type_cheq !='account_feed':
    ...
    def _get_payment_name(self, journal, date):
        ...
        if not sequ:
            sequ = [1]
        if journal and date:
            name = journal.code + "/" + str(date.year) + "/" + str(date.month) + "/" + str(len(sequ) + 1).zfill(5)
            if self.env['account.move'].search([('name', '=', name)]):
                name = journal.code + "/" + str(date.year) + "/" + str(date.month) + "/" + str(len(sequ) + 2).zfill(5)
    ...
    def _synchronize_from_moves(self, changed_fields):
        ...
        for pay in self.with_context(skip_account_move_synchronization=True):
            ...
            if 'line_ids' in changed_fields and pay.is_cheque == False and pay.state_cheque == 'draft':
                ...
                print(move_vals_to_write, " !!!!!!!!")
                move.write(move._cleanup_write_orm_values(move, move_vals_to_write))
                print("X")
                print(payment_vals_to_write, " ############3")
    ...
    def post_cheque(self):
        ...
        self.return_journal_id.action_post()

    def post_cheque_send(self):

        # if not self.journal_id.post_at_bank_rec:
        # ToDO : this is Abdulrhman Comment
        # ToDO : why you use self.move_id.post not self.action_post() ???
        # self.move_id.post()
        print("XXXXXXXXXXXXpost_cheque_sendXXXXXXXXXXX", self.type_cheq)
        if self.type_cheq in ['cash_feed', 'account_feed']:
            print("==================================>",self.type_cheq)
            lines = []
            second_journal_line = {
                'account_id': self.cash_journal_id.treasury_feed_account_id.id if self.type_cheq == 'cash_feed' else self.journal_cheque.account_feed_account_id.id ,
            ...
            print(first_journal_line)
            lines.append((0, 0, first_journal_line))
            move = self.env['account.move'].create({
                'date': self.date,
                'ref': "Cheque Num/" + self.cheque_no or '',
                'partner_id': self.vendor_id.id or '',
                'name': "confirm-feed " + self.env['ir.sequence'].next_by_code('cheque.feed.serial'),
                'company_id': self.company_id.id,
            ...
            self.state = 'posted'
            self.feed_confirm_journal_id = move.id
            move.action_post()
    ...
    def _get_reconsile(self, credit_account):
        ...
        if len(movee_line) > 1:
            ...
            if amls:
                ...
                self.env['account.full.reconcile'].create(vals)
    ...
    def create_journal_account_feed(self, journal, second_account):
        lines = []
        ...
        return lines
    def get_collect_form_bank_account_feed(self):
        move2 = self.env['account.move'].create({
            ...
            'company_id': self.company_id.id,
            'journal_id': self.journal_collection.id,
            'line_ids': self.create_journal_account_feed(self.journal_collection, self.journal_cheque.default_account_id),
            'cheque_number': self.cheque_no,
            'currency_id': self.currency_id.id,
        })
    ...
    def get_collect_form_bank_send_cheque(self):
        ...
        move2 = self.env['account.move'].create({
            ...
            'company_id': self.company_id.id,
            'journal_id': self.journal_collection.id,
            'line_ids': self.create_journal_send_state(self.journal_collection, self.journal_id.default_account_id),
            'cheque_number': self.cheque_no,
            'currency_id': self.currency_id.id,
        })
        print("XXXXXXXXXXXXXXX123XXXXXXXX ", move2)
    ...
    def send_feed(self):
        ...
        lines.append((0, 0, second_journal_line))
        first_journal_line = {
            'account_id': self.cash_journal_id.default_account_id.id if self.type_cheq == 'cash_feed' else self.account_transfer_id.id,
            'partner_id': self.vendor_id.id,
            'name': self.cheque_ref,
            'ref': self.cheque_no,
            'date_maturity': self.effective_date,
        ...
        move = self.env['account.move'].create({
            ...
            'ref': "Cheque Num/" + self.cheque_no or '',
            'partner_id': self.vendor_id.id or '',
            'name': "feed " + self.env['ir.sequence'].next_by_code('cheque.feed.serial'),
            'company_id': self.company_id.id,
            'journal_id': self.journal_cheque.id,
            'line_ids': lines,
            'cheque_number': self.cheque_no,
        ...
        self.state_cheque = 'feed'
        self.feed_journal_id = move.id
        move.action_post()
    ...
    def get_returned_to_customer_send(self):
        lines = []
        property_account = ''
        if self.account_med and self.check_mid == True and self.type_cheq == 'send_che':
            property_account = self.account_med_send
            second_journal_line = {
                'account_id': self.account_med_send.id,
                'partner_id': self.partner_id.id,
            ...
            elif self.type_cheq == 'account_feed':
                second_journal_line = {
    ...
    def save_payment(self):
        ...
        if self.state_cheque2 == 'under_collect':
            ...
            if self.type_cheq in ['cash_feed', 'send_che']:
                self.get_collect_form_bank_send_cheque()
            if self.type_cheq == 'account_feed':
                self.get_collect_form_bank_account_feed()
            self.state_cheque = 'reconciled'

        elif self.state_cheque2 == 'cancelled':
            if self.type_cheq == 'send_che' or self.type_cheq == 'cash_feed' or self.type_cheq == 'account_feed':
    ...
    # def ob_reset_draft(self):
    #     if self.amount <= 0:
    #         raise ValidationError(_("Enter Positive Amount"))
    #     lines = []
    #     print("XXXXXXXXXXXXXXXXXDDDDDDD ",self.type_cheq)
    #     if self.type_cheq == 'recieve_chq':
    #         print("11111111111")
    #         second_journal_line = {
    #             'account_id': self.partner_id.property_account_receivable_id.id ,
    ...
    #     move2 = self.env['account.move'].create(
    #         {'date': self.date,
    #          'ref': "Draft Cheque Num/" + self.cheque_no or '',
    #          'partner_id': self.partner_id.id or '',
    #          'name': self._get_payment_name(self.journal_id, self.date),
    #          'company_id': self.company_id.id,
    #          'journal_id': self.journal_id.id,
    #          'line_ids': lines,
    #          'cheque_number': self.cheque_no,
    #          'currency_id': self.currency_id.id,
    #     })
    #     self.return_journal_id = move2.id

    return_journal_id = fields.Many2one('account.move', 'return Journal')
    ...
    def mass_editing(self):
        for rec in self:
            ...
            if self.feed_confirm_journal_id:
                self.feed_confirm_journal_id.button_draft()
                self.feed_confirm_journal_id.unlink()
        # move_ids = self.env['account.move'].search([('cheque_number', '=', self.cheque_no)])
        # if move_ids:
        #     for record in move_ids:
        #         record.button_draft()
        #         record.button_cancel()
        # # if not self.return_journal_id:
        # #     self.ob_reset_draft()
        # if self.return_journal_id:
        #     self.return_journal_id.button_cancel()
        #     self.return_journal_id.button_draft()
