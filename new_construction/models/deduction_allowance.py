...
class DeductionLines(models.Model):
    ...
    @api.depends('precentage', 'contract_id.total_value','eng_template_id')
    def _calculate_down_payment(self):
        for rec in self:
            amount=0
            if rec.eng_template_id:
                for en in rec.eng_template_id.lines:
                    amount+=en.prec_price
                    rec.is_down_payment = (rec.precentage / 100) * amount
            else:
