...
class ResourceCalendar(models.Model):
    ...
    def att_get_work_intervals(self, day_start, day_end,tz):
        ...
        working_intervals = []
        print("SSSSSSSSSSSS",attendance_intervals)
        for interval in attendance_intervals:
            print("XXXXXXXXXXXXXXXXXX", interval)
            working_interval_tz = (interval[0].astimezone(pytz.UTC).replace(tzinfo=None), interval[1].astimezone(pytz.UTC).replace(tzinfo=None))
