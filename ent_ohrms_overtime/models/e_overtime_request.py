...
class HrOverTime(models.Model):
    ...
    def _onchange_date(self):
        ...
        if self.contract_id and self.date_from and self.date_to:
            overtime_dates = set((self.date_from + timedelta(days=x)).date() for x in range((self.date_to - self.date_from).days + 1))

            for leaves in self.contract_id.resource_calendar_id.global_leave_ids:
                leave_dates = set((leaves.date_from + timedelta(days=x)).date() for x in range((leaves.date_to - leaves.date_from).days + 1))
                
                if overtime_dates.intersection(leave_dates):
                    holiday = True
                    break
