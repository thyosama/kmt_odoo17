...
class Tender(models.Model):
    ...
    @api.depends('material_ids','equipment_ids', 'labour_ids', 'expense_ids', 'subconstractor_ids', 'equipment_ids', 'qty')
    ...
    @api.depends("material_ids",'total_material','total_labour','total_expense','total_subconstractor','total_equipment')
...
class Material(models.Model):
    ...
    @api.depends('planned_qty','cost_unit','currancy_id','ratio')
    def _compute_sub_total(self):
        for rec in self:
            rec.cost_subtotal =  rec.ratio*rec.cost_unit*rec.planned_qty
        ...
        if self.currancy_id and self.job_id.project_id:
            ...
            if self.job_id.project_id.currancy_ids :
                if self.job_id.project_id.currancy_ids.search([('currancy_id','=',self.currancy_id.id)],limit=1):
                    ...
                    .search([('currancy_id','=',self.currancy_id.id),('project_id','=',self.job_id.project_id.id)])
...
class Subconstractor(models.Model):
    ...
    product_id = fields.Many2one("product.product",string="Product",required=True,domain=[('subContractor','=',True)])
    description = fields.Char("Description")
    uom_id = fields.Many2one("uom.uom", string="Unit of Measure")
